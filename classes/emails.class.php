<?php
	...
	// require_once getcwd() . '/includes/constant.inc.php';
	require_once dirname(__DIR__) . '/includes/constant.inc.php';

	//	Defining constant to be used to send receive email	

	define("SITE_URL_ADDR", 			'fastlinky.com');

	define("SITE_URL_FULL", 			'https://fastlinky.com.org/');

	define("SITE_NAME_S", 				'Continue Content');

	

	define('TOEMAIL', 					'webtechhelp.org@gmail.com');
...
class Emails extends DatabaseConnection{
	...
	function SingleMailsend($fromMail, $toMail, $toName, $subject, $messageBody){

		$fromMail   = $fromMail;
		$from       = $this->FromName.'<'.$fromMail.'>';
		$to         = $toName.'<'.$toMail.'>';
	...
	//Add email details data on email_details table
	function emaildetails($toName, $toEmail, $Subject, $message, $From) {
		...
		$sql = 			"INSERT INTO email_details 
						(to_name, to_email, subject, message, from_email, added_on)
						 VALUES
						('$toName', '$toEmail', '$Subject', '$message', '$From', now())";
	...
	public function ShowSingleEmails(){
		$temp_arr = array();
		$res = "SELECT * FROM email_details order by id ASC";
	...
	//  Display email history  use
	public function ShowMailsbyCol($col, $colVal){
		$temp_arr = array();
		$sql = "SELECT * FROM email_details WHERE $col = '$colVal' ORDER BY id ASC";
		$sql.$this->conn->error;
		$query = $this->conn->query($sql);
	...
	/**
	*	Get email in an array
	*	@return array
	*/
	function getemailDetail($col, $colVal){
		
		$data		= array();
		
		$select     = "SELECT * FROM email_details WHERE $col = '$colVal'";
	...
	//Add email details data on allemail_details table
	function allemaildetails($toName, $toEmail, $Subject, $message, $From) {
		
		$sql = 			"INSERT INTO allemail_details 
						(to_name, to_email, subject, message, from_email, added_on)
						 VALUES
						('$toName', '$toEmail', '$Subject', '$message', '$From', now())";
	...
	public function ShowallEmaildetails(){
		...
		/**
	*	Get allemail information in an array
	*	@return array
	*/
	function getallemailDetail($id)
	{
		//declare var
		$data		= array();
		
		//statement
		$select     = "SELECT * FROM allemail_details WHERE id='$id'";
		//echo $select;
		//execute query
		$query      = $this->conn->query($select);
		$rows 		= $query->num_rows;
	...
	*/
	...
	function regEmailVerify($email,$name, $verLink)

	{
		...
		$heading  = "<h2 style='font:bold 14px Arial, Verdana, sans-serif;width:650px; height:40px;

								   background-color:#f93e4c; color:#fff; text-align:center; line-height:40px;

								   vertical-align:middle; padding:0; margin:1px'>
		...
		$this->sendEmail($to, $subject, $htmlBody, COMPANY_S, SITE_EMAIL);
	...
	*/
	...
	function regEmailVerifyWithPassword($email,$name, $verLink, $password)

	{
		...
		$defaultMesg = "
					   ...
					   <p class='blackLarge'>You have successfully registered with us, 
						...
						<p>Please find your password below:<br />
	...
	*/
	...
	function addAuthorEmail($credential, $work)

	{
		...
		$htmlBody .= "Dear Admin,
						...
						A new author has been added.

						Below is the detail of author.
		...
		$to = "ContinueContent Admin<info@continuecontent.com>";
	...
	*/
...

	{
		...
		$htmlBody .= "<p class='blackLarge'>".trim($mesg)."</p>" ;
		...
		$to = stripslashes($email)."<".stripslashes($email).">";

		# END OF MAIL TO

		// $autoresId    = "mousumijana3.7leelija@gmail.com";
		// $setUp        = "afgh@somedomain.com";

		//sending email

		// $this->sendEmail($to, $subject, $htmlBody, $autoresId, $setUp);
