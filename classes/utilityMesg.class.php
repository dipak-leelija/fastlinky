...
class MesgUtility extends Utility
{
	...
	*/
	function dispMessageS($typeM, $path, $style, $erBlock, $suBlock){
		...
		if(isset($_GET['msg'])){
			echo $this->renderMessageS($typeM, $_GET['msg'], $path, 'blackLarge', $erBlock, $suBlock);
		}
	}//eof
	...
	*/
	function showSuccessT($action, $id, $id_var, $url, $msg, $typeM) 
	{
		...
		if($id == 0)
		{
			header("Location: ".$url.$join."action=".$action."&typeM=".$typeM."&msg=".$msg);
		}
		else
		{
			header("Location: ".$url.$join."action=".$action."&typeM=".$typeM."&".$id_var."=".$id."&msg=".$msg);
		}
	}//eof
